...
# 📊 Анализ выполнения агентного пайплайна в Langfuse
...
## 1. Общий вид Traces в Langfuse

![Traces Overview](screenshots/table_tracing.png)

На скриншоте представлен общий список всех трейсов за последний день. Видно, что основной трейс `content_generation_workflow` выполняется успешно, а также присутствуют тестовые запуски.

---

## 2. Распределение событий по времени

![Chart Tracing](screenshots/chart_tracing.png)

График показывает распределение событий (Spans) по времени в течение дня. Видно, что основная активность приходится на утренние часы (8:44-8:48), что соответствует запуску пайплайна.

---

## 3. Детальный вид трейса

![Trace Details](screenshots/tracing.png)

На этом скриншоте представлен детальный вид трейса `content_generation_workflow` с полной цепочкой выполнения.

---

## 4. Полная цепочка вызовов (Trace)
...
## 5. Количество обращений к модели (LLM)
...
## 6. Время выполнения каждого агента
...
## 7. Общая задержка выполнения (Latency)
...
## 8. Количество входных и выходных токенов
